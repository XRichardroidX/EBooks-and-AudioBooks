...
class DemoBookReader extends StatefulWidget {
  ...
  @override
  _DemoBookReaderState createState() => _DemoBookReaderState();
}

class _DemoBookReaderState extends State<DemoBookReader> {
  ...
  int get _wordsPerPage => (numberOfWords / (_fontSize * 1.1)).round(); // Adjust words per page based on font size
  ...
  void _nextPage() {
    ...
    } else if (_currentPageIndex < maxPages - 1) {
      setState(() {
        _currentPageIndex++;
        _loadCurrentPage();
        _updateProgress();
        ...
        body: _isLoading
          ...
          onHorizontalDragEnd: (details) {
            if (details.primaryVelocity! < 0) {
              _nextPage(); // Swipe left to go to the next page
            } else if (details.primaryVelocity! > 0) {
              _previousPage(); // Swipe right to go to the previous page
            }
          },
              child: Container(
